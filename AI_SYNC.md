...
| 不动产制图 | ✅ | cartography 勘测定界图注记契约引擎（旋转矩形 SAT/边长中点法线零沿线偏移/点号角平分线朝外/左上起编/least_bad+inside_fallback 诚实标记）+ parcelmap GB/T 42547-2023 图 L.3 宗地图（SVG/PNG：界址点符号/红界址线/坐标表/分式/整百比例尺/「北」指北针/签注栏）+ cass 南方 CASS 联动（.dat 读写·注册表第 20 格式 + CASS 兼容 DXF SOUTH 编码 302001/302002）；CLI `render parcel-map`/`render parcel-dxf`；真实 CASS DXF 闭环验证 |
...
- 偏差：无
- 后续：§1.2 第 13 项余 ④壳层半（layoutview/工具箱面板）；宗地草图（B.4）/所有权宗地图（L.4）/房产图（L.5）图种待后续轮次
...
- 内容：kanyu-core 新模块 `cartography`（勘测定界图注记契约移植 realestate_map/label_placement.py：SAT 碰撞/边长中点法线零沿线偏移/点号角平分线朝外/左上起编/least_bad/inside_fallback）+ `cass`（CASS .dat 读写·注册表第 20 格式·CLI/MCP/壳层/Python/工具箱五处导出分派 + CASS 兼容 DXF SOUTH 编码 302001/302002 导出）；kanyu-render 新模块 `parcelmap`（GB/T 42547-2023 图 L.3 宗地图 SVG/PNG：坐标表/分式/整百比例尺/「北」指北针/签注栏）；CLI `render parcel-map`/`render parcel-dxf`（属性自动拾取 + 旗标覆盖）；introspect 角色串与 docs（ARCHITECTURE/API/CLI/MASTERPLAN/README/CHANGELOG）同步
- 真实验证：滨州 GB00032 真实 CASS DXF → 宗地图 1:700 十八注记**零压盖**（对照金样 stage10_all_157：边长 37.10/26.43/…/2.90 与分式 GB00032/0801、面积 3483.61 全一致）；CASS DXF 再导出实体结构与真实件一致（JZD 9CIRCLE+9TEXT/JZX 9+9/ZD 1/ZJ）且堪舆回读闭环；.dat CLI 导出/回读往返通过
- 偏差：① 坐标表轴序采测绘惯例 X=纵（北）Y=横（东）——金样 X=东系 QGIS 插件几何直取偏差（依据：parcel.rs 宗地 TXT / CASS .dat / TD/T 1008 惯例 + 带号冠于东坐标）；② `Command::Render` 改 `Box<RenderCommand>` 收纳（clippy large_enum_variant）；③ PNG 边长注记水平不旋转（已知限制，SVG 完整旋转，后续项）
- 后续：已同步 §1.2 第 13 项（kdb v2 多批次多图层/PNG 注记旋转/邻宗地注记/宗海用岛图种/壳层 MCP 投影/坐标表折列）

### [开工] 2026-08-28 kimi-code(main) — 不动产制图入内核：注记间距参考勘测定界图算法 + 国家标准布局 + CASS/CAD 联动
- 范围：crates/kanyu-core（parcel/新 cartography 模块、tooldef、toolrun、format）、crates/kanyu-render（layout/注记）、crates/kanyu-cli（命令）、docs（ARCHITECTURE/MASTERPLAN/API/CLI）
- 依据：用户目标（不动产制图迭代：注记间距参考堪舆工具箱勘测定界图算法、布局按国标、数据库按规划、CASS 接口 CAD 联动、真实数据验证、GitHub 迭代）；裁决 #18/#19/#20
- 预计：大（跨 crate + 文档 + 真实数据测试）
