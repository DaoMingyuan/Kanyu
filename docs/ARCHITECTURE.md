...
| `kanyu-render` | 眼睛：离屏地图渲染（SVG 零依赖 + tiny-skia PNG 光栅化，晨山/夜观星主题，属性驱动符号化 graduated/categorical）+ 宗地图渲染（`parcelmap`：GB/T 42547 图 L.3 版式，勘测定界图注记契约排版）；wgpu 实时管线待壳层 | 🚧 incubating | kanyu-core |
