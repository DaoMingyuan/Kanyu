...
| `kanyu-edit` | 手：Undo/Redo 框架（命令逆操作双栈 + 容量淘汰）+ 基础编辑命令（顶点移动/要素平移/删除/插入/属性更新，GeomPath 三级定位）+ Delta 快照 v2（FeatureDelta 增/删/改三态、DeltaSet 事务切片、命令/Delta 双通道统一 undo/redo、事务原子提交）+ DCEL v1（三表五指针、孔洞虚面约定、欧拉自检、面对角线分裂）；DCEL 续篇（外面接线/merge_faces）待后续 | 🚧 incubating | kanyu-core |
