...
|---|---|---|---|
| `kanyu-core` | 数据心脏：格式注册表（20 格式含自研 .kdb/.txt/.dat）、图层模型、空间分析（buffer/overlay/topology/sjoin/zonal）、QGIS 核心算法（geoprocess 一批：dissolve/simplify/centroid/convex_hull/delete_holes/explode/stats；二批：boundary/bounding_boxes/merge/extract_by_attribute/extract_by_location/count_points_in_polygon/field_stats/mean_coordinates；三批：distance_matrix/nearest_neighbor/multi_ring_buffer/variable_buffer/split_by_field/add_geometry_attributes/create_grid/points_along_lines/concave_hull/minimum_rotated_rect）、投影/度量、宗地 TXT（parcel）、不动产制图引擎（cartography：勘测定界图注记契约——边长中点法线/点号角平分线/SAT 避让）、南方 CASS 联动（cass：.dat 读写 + CASS 兼容 DXF SOUTH 编码）、字段计算器表达式引擎（attrcalc）、工具箱声明与统一执行（tooldef/toolrun，壳层/CLI/Python 三面投影）、CRS 全库检索（7507 条）、AGENTS.md 语义、系统自省 | ✅ stable | 无 |
...
| `kanyu-render` | 眼睛：离屏地图渲染（SVG 零依赖 + tiny-skia PNG 光栅化，晨山/夜观星主题，属性驱动符号化 graduated/categorical）+ 宗地图渲染（`parcelmap`：GB/T 42547 图 L.3 版式，勘测定界图注记契约排版）+ 宗海界址图渲染（`seamap`：图 L.7 版式，经纬网图廓 + DMS 坐标表）；wgpu 实时管线待壳层 | 🚧 incubating | kanyu-core |
| `kanyu-edit` | 手：Undo/Redo 框架（命令逆操作双栈 + 容量淘汰）+ 基础编辑命令（顶点移动/要素平移/删除/插入/属性更新，GeomPath 三级定位）+ Delta 快照 v2（FeatureDelta 增/删/改三态、DeltaSet 事务切片、命令/Delta 双通道统一 undo/redo、事务原子提交）+ DCEL v2（三表五指针、孔洞虚面约定、欧拉自检、面对角线分裂 + 外面/孔虚面 stub 环遍历 + merge_faces 单边合并逆操作）；DCEL 壳层接线与操作扩展待后续 | 🚧 incubating | kanyu-core |
| `kanyu-skill` | 技能：WASM 插件宿主（wasmtime 沙箱 + WIT 组件模型 ABI + fuel 配额）；MCP 热加载接线 📋 | 🚧 incubating | kanyu-core |
| `kanyu-shell` | 壳层：egui 深度桌面 UI（v0.9，ArcGIS Pro SDK 范式）——`commands.rs` 声明式命令注册表（DAML 思路：Ribbon/QAT/右键菜单统一投影，条件置灰）、Contents 图层面板（`toc.rs` 复选框/嵌套分组/符号化分类展开行/属性页四页签）、`dock.rs` 停靠系统（六面板三区拖拽停靠/浮动/关闭，跨区回流自适配+滚动契约）、中央视图停靠区（地图框页签吸附/浮动互转，画布纯白）、`symbology.rs` 符号化（单色/唯一值/分级设色，按层渲染叠图，入 .kyu）、`catalog.rs` 工程目录五分类（地图框/布局框/数据库/服务链接/本机数据）、`toolbox/` 工具箱（参数类型系统对齐 ArcGIS Python 工具箱规范：多值图层/线性单位/坐标系/校验分级/统一骨架/后台线程+进度模态可取消/终端三级日志）、`attrtable.rs` 属性表+字段计算器、`mapview.rs` 多地图视图 + `scene3d.rs` 实验性 3D 棱柱场景、`settings.rs`（坐标系全库搜索/渲染/界面缩放）、布局 v2（PNG 中文字体栈/入 .kyu/绑定地图框）+ 服务链接 v2（WFS 图层发现/WMS 底图叠加）、**不动产制图出图命令**（`estate_map`：九图种（宗地图/所有权/草图/房产/宗海三件套/用岛两件套）模态参数框 → kanyu-render 四渲染器落盘，属性自动拾取）、palette 语义色全套 + tokens::state 状态色派生 + WCAG 2.2 对比度强制、内置终端 + AI 对话 | 🚧 incubating | kanyu-core, kanyu-render, kanyu-skill |
