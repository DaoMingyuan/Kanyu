...
| `dsh/` | 堪舆 GIS × DeepSeek Harness 组件源：`plugin/`（Host+Client 双半，七大能力 + 9 个 kanyu_* 动态工具）/ `pkg/`（常驻静态插件适配器，装本机 web profile）/ `skills/`（WASM 技能 guest 源 + 组件化 .wasm）/ `presets/kanyu-gis/`（GIS 模式 preset + 领域技能）/ `tools/verify_preset.mjs` / `tools/test_plugin.mjs`（209 断言本地测试器）/ `sync-preset.sh`（同步本机安装区）；手册见 `docs/GIS_MODE.md` |
