...
- **桌面壳层**：`kanyu-shell` v0.9（ArcGIS Pro SDK 范式）——**地图框绑定自有图层**（切换框图层跟随、二维/三维分建、关闭≠删除）、声明式命令注册表（Ribbon/QAT/右键菜单统一投影）、Contents 图层面板（复选框显隐/图层组嵌套分组/符号化分类展开行/图层属性页）、六面板拖拽停靠系统（跨区回流自适配 + 滚动契约）、中央视图停靠区（地图框与布局页签吸附/浮动互转、纯白画布、多视图含实验性 3D 棱柱场景）、图层符号化（单色/唯一值分类/分级设色，入 `.kyu` 工程）、工程目录五分类（地图框/布局框（打印布局 v2，PNG 中文渲染/入 .kyu/绑定地图框）/数据库/服务链接（WFS 图层发现 + WMS 底图）/本机数据）、**编辑模式**（顶点拖拽/要素移动与增删/属性单元格编辑/Undo·Redo，kanyu-edit 内核）、QGIS 式工具箱（37 工具 5 分类，参数类型对齐 ArcGIS Python 工具箱规范 + 进度模态可取消 + 三级日志）、属性表（虚拟滚动/排序/字段计算器）、独立设置对话框（坐标系全库搜索/渲染/界面缩放）、三段式 Ribbon（悬停与页签动画）、独立终端 + AI 对话、UI 状态持久化、内置 `ui_kit` 设计系统（WCAG 2.2 对比度强制 + 状态色派生体系），晨山/夜观星双主题。
...
- **桌面端 MSI**：从 [Releases](https://github.com/DaoMingyuan/Kanyu/releases) 下载
  ...
  单入口：自动创建桌面「堪舆」快捷方式与开始菜单项（CLI/MCP 经 GUI 内置终端集成，
  不再提供独立终端入口）。
