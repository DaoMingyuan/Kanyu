...
- **宗海界址图渲染器 `seamap` + `kanyu render sea-boundary-map`**：
  ...
  注记顶/底横排、左右竖排，间隔 [1″…1°] 自适应 3~8 线/轴）+
  宗海图斑 RGB(245,162,122) + 0.5mm 红界址线 + 点号（无 J 前缀）/
  边长注记（cartography 排版）+ 右侧**界址点编号及坐标表**
  （点号|纬度（北纬）|经度（东经），度分秒 3 位小数进位安全，末行
  重复起点闭合，逐点经 `--source-epsg` 反算 EPSG:4490）+ 右下网格
  签注表（坐标系/高程基准固定值 + 六行签注）+ 左下整百比例尺 +
  右上指北针；SVG/PNG 双通道。真实宗地代理（GB00032，EPSG:4527）
  渲染 **DMS 坐标表与金样 stage10_all_161 逐行一致**。6 项测试
  （DMS 进位/网格间隔/全要素 SVG/PNG）。
...
- **宗地图四至/邻宗地注记（图 L.3 版面要素补全）**：`ParcelMapSpec`
  ...
  碰撞避让为**法向抬升 × 切向滑移二维逃逸**（宽文本块在短边上会被
  两角点号封锁切向全程，抬高法向净空后越过角点——GB32 北边界实案
  驱动），障碍含已放置点号/边长/四至注记与权属环，全灭取最小碰撞位。
  CLI `--sizhi-e/s/w/n` 旗标 + 属性 `ZDSZD/ZDSZN/ZDSZX/ZDSZB`（不动产
  登记数据库标准四至字段）自动拾取；MCP `kanyu_render_parcel_map`
  同名四参数。真实数据目检：GB00032 四至注记全部落位无压盖
  （北至注记西北高悬对齐金样观感）。cartography 新增公共助手
  `ring_edge_outward_normal`（环边外法线，环走向判定）。
...
- **宗地图第三轮：PNG 旋转注记 + 坐标表长表折列**：PNG 边长注记不再
  ...
  金样沿线竖排效果）；界址点坐标表长表自动折列（开发计划 §6/勘测
  定界图坐标表契约）——超出地图框内可用高时按 right_to_left 列流
  分栏（题行仅首列、表头每列重复、面积行仅末列、列间隔 4.0mm），
  折列后展示高度参与适配区/比例尺求解。新增 2 项测试（折列布局/
  旋转墨迹方向断言）；真实 60 界址点宗地渲染验证：61 行坐标表
  右起双列折列、GB00032 边长注记沿线旋转零回归。
...
- **`kanyu render layout` 布局排版（layout 排版器 CLI 出口）**：
  ...
  渲染；`--page a4l|a4p` / `--dpi` / `--no-legend` / `--no-scalebar` /
...
- **堪舆 GIS × DeepSeek Harness 组件源入库（`dsh/`）**：`plugin/host.js` +
  ...
  **kanyu_catalog 服务链接回执指引**：discover 附拉取用法指引、fetch
  ...
  测试器 99/99 全绿。**编辑页签联动刷新**：双端 Client 编辑页签
  ...
  **查询结果联动属性表**：双端数据页签 runQuery 命中即自动载入结果集
  属性表（免二次点击），测试器 108/108 全绿。**目录页签 freshness 自动
  重扫**：当前图层变为清单外新文件时自动重扫（knownRef 防重复），产出类
  操作广播后目录不再滞留，测试器 110/110 全绿。**地图页签联动重渲染**：
  store.rev 内容版本号 + 编辑四入口递增广播 + TabMap 已渲染过自动跟随
  重渲，测试器 112/112 全绿。**3D 页签联动重载**：Tab3d 已加载场景跟随
  rev/path 自动重载（TabMap 同范式），测试器 114/114 全绿。**双半盘点 +
  RPC 面对称锁**：动态/静态两半 RPC 方法集 18=18 零独有，差异白名单
  （四处设计意图）入 dsh/README，测试器新增对称断言，115/115 全绿。
